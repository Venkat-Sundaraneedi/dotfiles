...
2. Apply Boring Rust patterns and established project conventions
...
cargo check                # Verify everything works
```
...
2. Use `cargo add <package>` to add dependencies (auto-adds latest version)
3. Use `cargo remove <package>` to remove dependencies — never manually edit Cargo.toml
4. Check `clippy.toml` for complexity thresholds
...
**Before committing:**
1. Run `cargo clippy --all-targets -- -D warnings && cargo test`
2. Ensure no `#[allow]` without justification comment
...
- **rust-async-patterns** skill — Deep async Rust patterns with Tokio, channels, streams, and concurrent patterns
